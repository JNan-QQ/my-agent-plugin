...
| 首批合并文件不存在 | sub-agent 写入路径不一致 | 确认 {PROJECT_DIR}/.tmp/ 目录存在，检查文件名 | 手动指定正确路径重试 |
...
| doc2md 转换失败 | .docx 损坏/加密/格式异常 | 检查文件是否可打开，重试1次 | 提示用户手动复制内容为 .md |
...
| `.pdf` | 提示用户转换为 .docx 或复制内容（doc2md 仅支持 .docx） |
...
调用 `get_file_hash` 获取文件 MD5，检查是否已转换过：
```json
{"name": "get_file_hash", "arguments": {"path": "<输入文件路径>.docx"}}
```
若 `{PROJECT_DIR}/.tmp/` 下存在 `{hash}.md`，直接复用，跳过转换。
...
> `extract_images: true` 会在输出目录生成 `images/` 子目录，后续分析图片时需引用此路径。

**Step 3：验证转换结果**

转换完成后读取 `.md` 文件，确认内容非空且包含需求文本。若文件为空或报错，提示用户检查 .docx 文件是否损坏。
...
| 用例生成后不验证JSON | 格式错误导致后续失败 | 用 python json.load 验证 |
| 跳过场景覆盖检查 | 用例不完整 | 检查正常+异常+边界三类 |
...
| 预期结果省略具体描述 | 不可验证 | 完整写入所有细节 |
| type字段使用非标准值 | 场景分类错误 | 使用正常流程/异常流程/边界条件 |
...
| 不按项目目录输出 | 文件混乱 | 所有产物输出到{PROJECT_DIR} |

### 典型错误示例

**❌ 复述分析过程**：需求分析师首先检查了功能列表，发现3个模块，然后对每个模块进行了详细分析……（500字）
**✅ 只转达结果**：需求分析完成：3个模块、12条需求、2个风险点。文件：{PROJECT_DIR}/requirements/REQ-20260602-1430.md

**❌ markdown包裹JSON**：```json\n{"stage":2,...}\n```
**✅ 纯JSON**：{"stage":2,"verdict":"通过",...}

**❌ 预期结果省略**：登录成功，显示首页
**✅ 预期结果完整**：跳转到首页 http://example.com/dashboard，顶部显示"欢迎 admin"，右上角显示用户头像
